...
DisplayPort::DisplayPort(int mosi, int scl, int dc, int cs, int rst, int width, int height, spi_host_device_t spihost)
  ...
  DisplayLen = transfer_bytes;  //(1byte 8ipex)
  DispBuffer = (uint8_t *)heap_caps_malloc(DisplayLen, MALLOC_CAP_SPIRAM);
  TxBuffer = (uint8_t *)heap_caps_malloc(DisplayLen, MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
  assert(DispBuffer);
  assert(TxBuffer);
...
void DisplayPort::RLCD_Sendbuffera(uint8_t *Data, int len) {
  if (len > DisplayLen) {
    ESP_LOGE(TAG, "Display transfer is too large: %d", len);
    return;
  }

  memcpy(TxBuffer, Data, len);
  esp_err_t err = esp_lcd_panel_io_tx_color(io_handle, -1, TxBuffer, len);
  if (err != ESP_OK)
