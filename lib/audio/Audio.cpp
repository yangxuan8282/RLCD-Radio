...
// Wskazujemy kompilatorowi, że te zmienne istnieją w naszym main.cpp
extern volatile int16_t shared_pcm_buffer[];
extern volatile bool shared_data_ready;
extern volatile uint32_t callback_trigger_count;
#define CORE_FFT_SAMPLES 256

void audio_process_raw_samples(int32_t* outBuff, int16_t validSamples) {
    callback_trigger_count = callback_trigger_count + 1;

    if (!shared_data_ready && validSamples >= CORE_FFT_SAMPLES) {
        for (int i = 0; i < CORE_FFT_SAMPLES; i++) {
            int16_t left = (int16_t)((outBuff[i * 2] >> 16) & 0xFFFF);
            int16_t right = (int16_t)((outBuff[i * 2 + 1] >> 16) & 0xFFFF);
            
            if (left == 0 && right == 0) {
                left = (int16_t)(outBuff[i * 2] & 0xFFFF);
                right = (int16_t)(outBuff[i * 2 + 1] & 0xFFFF);
            }
            shared_pcm_buffer[i] = (left + right) / 2;
        }
